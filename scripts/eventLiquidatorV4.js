...
// ============================================================
// ⚡ EVENT LIQUIDATOR V4 - Profit Simulation + Auto-Withdraw
// ============================================================
...
async function simulateProfit(chain, pos, collateral, debt) {
  ...
  // Get current prices
  const ethPrice = currentPrices[`${chain}-ETH/USD`] || 2900;
  ...
  const debtToCover = debtAmountRaw * 0.5; // Liquidate 50%
  
  // Estimate collateral received (debt value + liquidation bonus)
  const liquidationBonus = collateralAsset.bonus / 10000; // e.g., 500 = 5%
  const collateralReceived = debtToCover * (1 + liquidationBonus);
  
  // Estimate swap slippage (0.5% for most pairs, 1% for exotic)
  ...
  // Gas estimation
  const gasUnits = 550000; // Typical for flash liquidation
  const gasPrice = await getGasPrice(chain);
  const gasCostNative = gasUnits * gasPrice / 1e9;
  const gasCostUsd = gasCostNative * (config.nativePrice || ethPrice);
  
  // Calculate profit
  const grossProfit = swapOutput - debtToCover;
  const grossProfitUsd = grossProfit * pos.debt / debtAmountRaw; // Scale to USD
...
async function getGasPrice(chain) {
  try {
    ...
    return Number(feeData.gasPrice) / 1e9; // Gwei
  } catch {
...
╔══════════════════════════════════════════════════════════════════════╗
║  ⚡ EVENT LIQUIDATOR V4 - Profit Simulation + Auto-Withdraw          ║
║  🎯 Skips unprofitable liquidations | Min profit: $${MIN_PROFIT_USD}              ║
╚══════════════════════════════════════════════════════════════════════╝
`);
...
async function executeLiquidation(pos) {
  ...
  try {
    if (protocol === 'aave') {
      // Detect collateral and debt assets
      ...
      if (!simulation.profitable) {
        console.log(`   ⏭️ SKIPPED: Net profit $${simulation.netProfitUsd?.toFixed(2)} < $${MIN_PROFIT_USD} minimum`);
        stats.skipped++;
        await sendDiscord(`⏭️ SKIPPED LIQUIDATION\n${chain} | ${user.slice(0,10)}...\nDebt: $${debt.toFixed(0)}\nNet profit: $${simulation.netProfitUsd?.toFixed(2)} (below $${MIN_PROFIT_USD} min)`, false);
        return { success: false, reason: 'unprofitable' };
      }
      
      await sendDiscord(`💀 EXECUTING LIQUIDATION!\n${chain} ${protocol}\nUser: ${user.slice(0,10)}...\nDebt: $${debt.toFixed(0)} | HF: ${hf.toFixed(4)}\nExpected profit: $${simulation.netProfitUsd?.toFixed(2)}`, true);
      ...
      // Compound liquidations (no profit simulation for now)
...
async function processResults(results) {
  ...
  if (liquidatable.length > 0) {
    console.log(`\n🔥🔥🔥 ${liquidatable.length} LIQUIDATABLE - SIMULATING PROFITS 🔥🔥🔥`);
    await sendDiscord(`🔥 ${liquidatable.length} POSITIONS LIQUIDATABLE - CHECKING PROFITABILITY`, true);
...
async function sendDiscord(message, urgent = false) {
  ...
  try {
    await fetch(DISCORD_WEBHOOK, {
      ...
      body: JSON.stringify({ content: urgent ? '@here ' + message : message, username: '⚡ Liquidator V4' }),
    });
  } catch {}
}
