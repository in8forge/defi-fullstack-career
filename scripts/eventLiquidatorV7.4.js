...
// ============================================================
// ⚡ EVENT LIQUIDATOR V7.4 - DYNAMIC ASSET & PRICE MANAGEMENT
// ============================================================
// V7.4 Changes:
// + Full asset coverage - Dynamic Aave asset discovery per chain
// + Venus borrower discovery - Query Venus Borrow events weekly
// + Dynamic price fetching - Chainlink oracles, no hardcoded prices
// + Unknown pair fallback - Log opportunity, don't waste gas
// + Swap path validation - Check DEX liquidity before executing
...
const CHAINLINK_FEEDS = {
  base: {
    ...
    '0xcbB7C0000aB88B473b1f5aFd9ef808440eed33Bf': '0x64c911996D3c6aC71E9b8Bd43da7f6B7F2D3d3C9', // cbBTC
  },
  polygon: {
    ...
    '0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6': '0xc907E116054Ad103354f2D350FD2514433D57F6f', // WBTC
  ...
  arbitrum: {
    ...
    '0x2f2a2543B76A4166549F7aaB2e75Bef0aefC5B0f': '0xd0C7101eACbB49F3deCcCc166d238410D6D46d57', // WBTC
  ...
  avalanche: {
    ...
    '0x49D5c2BdFfac6CE2BFdB6640F4F80f226bc10bAB': '0x976B3D034E162d8bD72D6b9C989d545b839003b0', // WETH.e
    '0x50b7545627a5162F82A992c33b87aDc75187B218': '0x2779D32d5166BAaa2B2b658333bA7e6Ec0C65743', // WBTC.e
    '0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7': '0xEBE676ee90Fe1112671f19b6B7459bC678B67e8a', // USDT
  },
  bnb: {
...
const DEX_ROUTERS = {
  ...
  bnb: {
    pancakeV3: '0x13f4EA83D0bd40E75C8222255bc855a974568Dd4',
...
const MIN_SWAP_OUTPUT_RATIO = 0.95; // Accept up to 5% slippage
...
const VENUS_ABI = ['function getAccountLiquidity(address) view returns (uint256, uint256, uint256)', 'function getAllMarkets() view returns (address[])'];
const VTOKEN_ABI = [
  'function balanceOf(address) view returns (uint256)', 
  'function borrowBalanceStored(address) view returns (uint256)',
  'event Borrow(address borrower, uint256 borrowAmount, uint256 accountBorrows, uint256 totalBorrows)',
];
const ERC20_ABI = ['function balanceOf(address) view returns (uint256)', 'function transfer(address, uint256) returns (bool)', 'function decimals() view returns (uint8)', 'function symbol() view returns (string)'];
...
const UNISWAP_V3_POOL_ABI = ['function liquidity() view returns (uint128)', 'function slot0() view returns (uint160 sqrtPriceX96, int24 tick, uint16 observationIndex, uint16 observationCardinality, uint16 observationCardinalityNext, uint8 feeProtocol, bool unlocked)'];
const UNISWAP_FACTORY_ABI = ['function getPool(address tokenA, address tokenB, uint24 fee) view returns (address pool)'];
...
let stats = { events: 0, checks: 0, liquidations: 0, badDebt: 0, attempted: 0, failed: 0, withdrawn: 0, skippedUnprofitable: 0, skippedNoLiquidity: 0, unknownPairs: 0 };
let venusProvider, venusWallet, venusWsProvider, venusComptroller, venusLiquidator;
let liquidatorAddresses = {};
let currentPrices = {}; // token address -> USD price
let priceFeeds = {}; // chain -> { tokenAddress -> feedContract }
let validatedSwapPaths = {}; // cache: `${chain}-${tokenA}-${tokenB}` -> { valid: bool, liquidity: number }
let unknownPairOpportunities = []; // logged opportunities we couldn't execute
...
`);
  ...
  // Discover all Aave assets dynamically
  ...
  console.log(`🔗 SWAP PATH VALIDATION: Enabled\n`);
}

// ============================================================
// DYNAMIC AAVE ASSET DISCOVERY
// ============================================================
    ...
    try {
      ...
      for (const reserve of reserves) {
        try {
          ...
          // Get token addresses (aToken, stableDebt, variableDebt)
          ...
          assets.push({
            ...
            stableDebtToken: tokenAddresses.stableDebtTokenAddress,
...
// ============================================================
// DYNAMIC CHAINLINK PRICE FETCHING
// ============================================================
  ...
  for (const [chain, feeds] of Object.entries(CHAINLINK_FEEDS)) {
    ...
    for (const [tokenAddress, feedAddress] of Object.entries(feeds)) {
      try {
        ...
        currentPrices[`${chain}-${tokenAddress.toLowerCase()}`] = Number(price) / 1e8;
      } catch (e) {
        // Feed unavailable
      }
    }
    
    const feedCount = Object.keys(priceFeeds[chain]).length;
    console.log(`   ✅ ${chain}: ${feedCount} price feeds initialized`);
  }
}
...
async function fetchPrice(chain, tokenAddress) {
  ...
  // Check cache first (prices updated by oracle events)
  if (currentPrices[key] && Date.now() - (currentPrices[`${key}-timestamp`] || 0) < 60000) {
  ...
  if (feed) {
    try {
      ...
      // Validate price freshness (within 1 hour)
      const age = Date.now() / 1000 - Number(updatedAt);
      if (age > 3600) {
        console.log(`   ⚠️  Stale price for ${tokenAddress.slice(0,10)}... (${Math.floor(age/60)}min old)`);
      }
      
      currentPrices[key] = priceUsd;
      currentPrices[`${key}-timestamp`] = Date.now();
      return priceUsd;
    } catch {}
  }
  
  // Fallback: check if it's a stablecoin
  ...
  if (asset && stablecoins.includes(asset.symbol)) {
    return 1.0;
  }
...
async function fetchAllPrices(chain) {
  const assets = CHAIN_ASSETS[chain] || [];
  const prices = {};
  ...
  for (const asset of assets) {
    ...
    if (price !== null) {
      prices[asset.token.toLowerCase()] = price;
...
async function validateSwapPath(chain, tokenIn, tokenOut, amountIn) {
  ...
  if (!dex?.quoter) {
    // No quoter available, assume path is valid but flag uncertainty
    return { valid: true, uncertain: true, liquidity: 0 };
  }
  
  const provider = providers[chain];
  if (!provider) return { valid: false, reason: 'No provider' };
  
  try {
    const quoter = new ethers.Contract(dex.quoter, UNISWAP_QUOTER_ABI, provider);
    ...
    if (bestOutput === 0n) {
      validatedSwapPaths[cacheKey] = { valid: false, reason: 'No liquidity pool found', timestamp: Date.now() };
      return validatedSwapPaths[cacheKey];
    }
    
    // Calculate effective price ratio
    const inputDecimals = await getDecimals(chain, tokenIn);
    const outputDecimals = await getDecimals(chain, tokenOut);
    
    const inputPrice = await fetchPrice(chain, tokenIn);
    const outputPrice = await fetchPrice(chain, tokenOut);
    
    if (inputPrice && outputPrice) {
      const expectedOutput = (Number(amountIn) / 10**inputDecimals) * inputPrice / outputPrice * 10**outputDecimals;
      const actualOutput = Number(bestOutput);
      const ratio = actualOutput / expectedOutput;
      
      if (ratio < MIN_SWAP_OUTPUT_RATIO) {
        validatedSwapPaths[cacheKey] = { 
          valid: false, 
          reason: `High slippage (${((1 - ratio) * 100).toFixed(2)}%)`,
          timestamp: Date.now()
        };
        return validatedSwapPaths[cacheKey];
      }
    ...
    const liquidityUsd = inputPrice ? (Number(amountIn) / 10**inputDecimals) * inputPrice * 10 : MIN_LIQUIDITY_USD; // Rough estimate
...
async function getDecimals(chain, tokenAddress) {
  ...
  try {
    const token = new ethers.Contract(tokenAddress, ERC20_ABI, providers[chain]);
...
function logUnknownPairOpportunity(chain, protocol, pos, collateral, debt, reason) {
  ...
  const opportunity = {
    ...
    healthFactor: pos.hf,
    debtUsd: pos.debt,
    collateralToken: collateral?.asset?.token || collateral?.vToken || 'unknown',
    collateralSymbol: collateral?.asset?.symbol || collateral?.vToken || 'unknown',
    debtToken: debt?.asset?.token || debt?.config?.underlying || 'unknown',
  ...
  if (unknownPairOpportunities.length > 100) {
    unknownPairOpportunities = unknownPairOpportunities.slice(-100);
  }
  
  console.log(`   📝 LOGGED UNKNOWN PAIR: ${chain} ${protocol}`);
  console.log(`      User: ${pos.user.slice(0, 12)}... | HF: ${pos.hf.toFixed(4)} | Debt: $${pos.debt.toFixed(0)}`);
  console.log(`      Collateral: ${opportunity.collateralSymbol} | Debt: ${opportunity.debtSymbol}`);
  console.log(`      Reason: ${reason}`);
  
  // Save to file
  try {
    fs.writeFileSync('data/unknown_pairs.json', JSON.stringify(unknownPairOpportunities, null, 2));
  } catch {}
  
  // Send Discord alert for high-value opportunities
  if (pos.debt > 10000) {
    sendDiscord(`📝 UNKNOWN PAIR OPPORTUNITY\n${chain} ${protocol}\nUser: ${pos.user.slice(0, 12)}...\nDebt: $${pos.debt.toFixed(0)}\n${opportunity.collateralSymbol} → ${opportunity.debtSymbol}\nReason: ${reason}`, false);
  }
}

// ============================================================
// VENUS BORROWER DISCOVERY (NEW)
// ============================================================
...
async function discoverVenusBorrowers() {
  ...
  const fromBlock = currentBlock - 200000; // ~1 week on BSC
  ...
  for (const [symbol, config] of Object.entries(VENUS_VTOKENS)) {
    try {
      ...
      const events = await vToken.queryFilter(borrowFilter, fromBlock, currentBlock);
  ...
  console.log(`\n   📊 Venus: +${newBorrowers} new borrowers (${borrowers.venus.size} total)`);
  
  // Save Venus borrowers
  try {
    const venusData = Array.from(borrowers.venus);
    fs.writeFileSync('data/venus_borrowers.json', JSON.stringify(venusData, null, 2));
    console.log('   ✅ Saved venus_borrowers.json');
  } catch {}
}
...
async function loadBorrowers() {
  ...
  // Load Venus borrowers
  try {
    const venusData = JSON.parse(fs.readFileSync('data/venus_borrowers.json', 'utf8'));
    borrowers.venus = new Set(venusData);
    console.log(`   📊 Loaded ${borrowers.venus.size} Venus borrowers from file`);
  } catch {}
}

// ============================================================
// PROFIT SIMULATION (Updated with dynamic prices)
// ============================================================
...
async function estimateGasCost(chain) {
  ...
  // Get native token price dynamically
  let nativePrice = config.nativePrice;
  if (chain === 'bnb') {
    const bnbPrice = await fetchPrice('bnb', '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c');
    if (bnbPrice) nativePrice = bnbPrice;
  } else {
    const nativeTokens = {
    ...
    if (nativeTokens[chain]) {
      const price = await fetchPrice(chain, nativeTokens[chain]);
      if (price) nativePrice = price;
    }
  }
...
// ============================================================
// COLLATERAL & DEBT DETECTION (Updated with dynamic prices)
// ============================================================

async function hasAaveCollateral(chain, user) {
  const assets = CHAIN_ASSETS[chain];
  if (!assets || !assets.length) return { has: false };
  ...
  try {
    ...
    for (let i = 0; i < results.length; i++) {
      if (results[i].success) {
        ...
        if (bal > 0n) {
          const price = await fetchPrice(chain, assets[i].token);
          const balUsd = price ? Number(bal) / 10**assets[i].decimals * price : 0;
  ...
  try {
    ...
    for (let i = 0; i < results.length; i++) {
      if (results[i].success) {
        ...
        if (bal > 0n) {
          const price = await fetchPrice(chain, assets[i].token);
          const debtUsd = price ? Number(bal) / 10**assets[i].decimals * price : 0;
...
// ============================================================
// EXECUTION LOGIC (Updated with swap path validation)
// ============================================================
...
async function executeAaveLiquidation(chain, pos) {
  ...
  try {
    ...
    // SWAP PATH VALIDATION
    const swapCheck = await validateSwapPath(
      chain, 
    ...
    if (swapCheck.uncertain) {
      console.log(`   ⚠️  Swap path uncertain, proceeding with caution`);
    }
...
async function executeCompoundLiquidation(chain, market, pos) {
  ...
  try {
    ...
    // Validate swap paths for all collaterals
    let hasValidPath = false;
    for (const collateral of marketConfig.collaterals) {
      const swapCheck = await validateSwapPath(chain, collateral.token, marketConfig.baseToken, ethers.parseEther('1'));
      if (swapCheck.valid) {
        hasValidPath = true;
        break;
      }
    ...
    if (!hasValidPath) {
      logUnknownPairOpportunity(chain, 'compound', pos, { asset: { symbol: 'MULTI' } }, { asset: { symbol: market } }, 'No valid swap path for collaterals');
      return;
    }
...
async function executeVenusLiquidation(pos) {
  ...
  try {
    ...
    // Validate swap path
    const swapCheck = await validateSwapPath('bnb', collateral.config.underlying, debt.config.underlying, collateral.balance);
    if (!swapCheck.valid && collateral.config.underlying !== debt.config.underlying) {
      logUnknownPairOpportunity('bnb', 'venus', pos, collateral, debt, swapCheck.reason || 'No swap path');
      return;
    }
...
// ============================================================
// BORROWER DISCOVERY (Updated to include Venus)
// ============================================================
...
async function subscribeToOracles() {
  ...
  for (const [chain, feeds] of Object.entries(CHAINLINK_FEEDS)) {
    ...
    for (const [tokenAddress, feedAddress] of Object.entries(feeds)) {
      try {
        ...
        feed.on('AnswerUpdated', (newPrice) => {
          stats.events++;
          currentPrices[`${chain}-${tokenAddress.toLowerCase()}`] = Number(newPrice) / 1e8;
          currentPrices[`${chain}-${tokenAddress.toLowerCase()}-timestamp`] = Date.now();
          
          // Trigger checks on significant price movement
          if (chain === 'bnb') {
            checkVenus();
          } else {
    ...
    console.log(`   ✅ ${chain}: ${subscribedCount} price feeds subscribed`);
  }
}
...
async function sendDiscord(message, urgent = false) {
  ...
  try { 
    await fetch(DISCORD_WEBHOOK, { 
      ...
      body: JSON.stringify({ content: urgent ? '@here ' + message : message, username: '⚡ Liquidator V7.4' }) 
    }); 
  } catch {}
}
...
async function main() {
  ...
  await sendDiscord(`🚀 LIQUIDATOR V7.4 STARTED\n⚡ Aave + Compound + Venus\n🛡️ MEV Protection: ON\n📈 Profit Sim: ON\n💰 Auto-withdraw: ON\n🔗 Swap Validation: ON\n🪙 Dynamic Prices: ON`, true);
  
  console.log('🚀 Listening for events...\n');
  
  setInterval(() => {
    console.log(`[${new Date().toLocaleTimeString()}] Events: ${stats.events} | Checks: ${stats.checks} | Attempted: ${stats.attempted} | Success: ${stats.liquidations} | Failed: ${stats.failed} | Skipped: ${stats.skippedUnprofitable} | NoLiquidity: ${stats.skippedNoLiquidity} | UnknownPairs: ${stats.unknownPairs}`);
  }, 60000);
  
  setInterval(backgroundScan, 30000);
  
  // Refresh asset discovery monthly
  setInterval(discoverAllAaveAssets, 30 * 24 * 60 * 60 * 1000);
