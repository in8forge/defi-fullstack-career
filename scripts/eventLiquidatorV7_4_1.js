...
// ============================================================
// ⚡ EVENT LIQUIDATOR V7.4.1 - SPEED OPTIMIZED
// ============================================================
// V7.4.1 Changes:
// + FIXED: Interval overflow bug (30 days > 32-bit int)
// + Parallel price fetching - All prices fetched concurrently
// + Pre-built TX templates - Ready-to-sign for common pairs
// + Latency logging - Identify bottlenecks in real-time
// + Smart MEV skip - Direct submission for <$500 (speed > protection)
// + Asset discovery runs ONCE on startup, weekly refresh
...
let stats = { events: 0, checks: 0, liquidations: 0, badDebt: 0, attempted: 0, failed: 0, withdrawn: 0, skippedUnprofitable: 0, skippedNoLiquidity: 0, unknownPairs: 0, mevSkipped: 0 };
...
async function fetchPrice(chain, tokenAddress) {
  ...
  if (currentPrices[key] && Date.now() - (currentPrices[`${key}-ts`] || 0) < 60000) {
...
function logUnknownPairOpportunity(chain, protocol, pos, collateral, debt, reason) {
  ...
  if (unknownPairOpportunities.length > 100) unknownPairOpportunities = unknownPairOpportunities.slice(-100);
...
async function executeCompoundLiquidation(chain, market, pos) {
  ...
  try {
    ...
    const tx = await liquidator.executeLiquidationAll(marketConfig.comet, pos.user, baseAmount, { gasLimit: GAS_CONFIG[chain].gasLimit });
    console.log(`   ⏳ TX: ${tx.hash}`);
    const receipt = await tx.wait();
    ...
    for (const [tokenAddress, feedAddress] of Object.entries(feeds)) {
      try {
        ...
        feed.on('AnswerUpdated', (newPrice) => {
          ...
          if (chain === 'bnb') checkVenus();
          else checkAllProtocols(chain);
        });
...
async function sendDiscord(message, urgent = false) {
  ...
  try { 
    await fetch(DISCORD_WEBHOOK, { 
      ...
      body: JSON.stringify({ content: urgent ? '@here ' + message : message, username: '⚡ Liquidator V7.4.1' }) 
    }); 
  } catch {}
}
...
async function main() {
  ...
  await sendDiscord(`🚀 LIQUIDATOR V7.4.1 STARTED\n⚡ Aave + Compound + Venus\n🛡️ Smart MEV: <$${MEV_THRESHOLD_USD} = direct\n📈 Profit Sim: ON\n💰 Auto-withdraw: ON\n⚡ TX Templates: ${Object.keys(txTemplates).length}`, true);
  
  console.log('🚀 Listening for events...\n');
  
  // Stats + latency report every minute
  setInterval(() => {
    console.log(`[${new Date().toLocaleTimeString()}] Events: ${stats.events} | Checks: ${stats.checks} | Attempted: ${stats.attempted} | Success: ${stats.liquidations} | Failed: ${stats.failed} | Skipped: ${stats.skippedUnprofitable} | MEV-Skip: ${stats.mevSkipped}`);
  }, 60000);
  
  // Latency report every 5 minutes
  setInterval(logLatencyReport, 300000);
