...
// ============================================================
// ⚡ EVENT LIQUIDATOR V6.1 - Bad Debt Filter + Multi-Protocol
// ============================================================
...
const VENUS_VTOKENS = {
  ...
  vDAI: { address: '0x334b3eCB4DCa3593BCCC3c7EBD1A1C1d1780FBF1', symbol: 'DAI', decimals: 8 },
  vBUSD: { address: '0x95c78222B3D6e262426483D42CfA53685A67Ab9D', symbol: 'BUSD', decimals: 8 },
};
...
const VENUS_COMPTROLLER_ABI = ['function getAccountLiquidity(address) view returns (uint256, uint256, uint256)'];
...
let badDebtAddresses = new Set(); // Track known bad debt
let stats = { events: 0, checks: 0, liquidations: 0, skipped: 0, badDebt: 0, venusPositions: 0 };
...
async function initVenus(pk, liquidatorAddresses) {
  ...
  try {
    ...
    if (liquidatorAddresses.bnb) {
      venusLiquidator = liquidatorAddresses.bnb;
    }
...
// ============================================================
// BAD DEBT DETECTION - CRITICAL FOR ALL CHAINS
// ============================================================
...
async function hasAaveCollateral(chain, user) {
  ...
  try {
    ...
    for (const r of results) {
      if (r.success) {
        const bal = iface.decodeFunctionResult('balanceOf', r.returnData)[0];
...
async function hasCompoundCollateral(chain, market, user) {
  const cometAddress = COMPOUND_MARKETS[chain]?.[market];
  if (!cometAddress) return false;
  
  try {
    const comet = new ethers.Contract(cometAddress, [
      'function collateralBalanceOf(address, address) view returns (uint128)',
      'function numAssets() view returns (uint8)',
      'function getAssetInfo(uint8) view returns (tuple(uint8, address, address, uint64, uint64, uint64, uint128, uint128))',
    ], providers[chain]);
    ...
    for (let i = 0; i < numAssets; i++) {
      try {
        ...
        const collateral = await comet.collateralBalanceOf(user, assetInfo[1]);
        if (collateral > 0n) return true;
      } catch {}
    }
  } catch {}
...
// ============================================================
// PROTOCOL CHECKS WITH BAD DEBT FILTER
// ============================================================
  ...
  try {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      try {
        ...
        // BAD DEBT CHECK: Must have collateral to be profitable
        if (liquidatable && collateralUsd < 10) {
          if (!badDebtAddresses.has(`aave-${chain}-${user}`)) {
            badDebtAddresses.add(`aave-${chain}-${user}`);
            stats.badDebt++;
            console.log(`   💀 BAD DEBT: ${chain} aave ${user.slice(0, 10)}... | $${debtUsd.toFixed(0)} debt, $${collateralUsd.toFixed(0)} collateral`);
          }
          continue; // Skip bad debt
        }
  ...
  try {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      try {
        ...
        // BAD DEBT CHECK for Compound
        if (isLiq) {
          const hasCollateral = await hasCompoundCollateral(chain, market, user);
          if (!hasCollateral) {
            if (!badDebtAddresses.has(`compound-${chain}-${user}`)) {
              badDebtAddresses.add(`compound-${chain}-${user}`);
              stats.badDebt++;
              console.log(`   💀 BAD DEBT: ${chain} compound ${user.slice(0, 10)}... | $${debt.toFixed(0)} debt, no collateral`);
            }
            continue;
          }
        }
...
async function checkVenusAccount(address) {
  try {
    ...
    // BAD DEBT CHECK for Venus
    if (liquidatable) {
      const hasCollateral = await hasVenusCollateral(address);
      if (!hasCollateral) {
        if (!badDebtAddresses.has(`venus-${address}`)) {
          badDebtAddresses.add(`venus-${address}`);
          stats.badDebt++;
          console.log(`   💀 BAD DEBT: bnb venus ${address.slice(0, 10)}... | $${shortfallUsd.toFixed(0)} shortfall, no collateral`);
        }
        return null; // Skip bad debt
      }
    }
...
async function scanVenus() {
  ...
  for (const address of borrowers.venus) {
    ...
    if (account) {
      if (account.liquidatable || (account.liquidity > 0 && account.liquidity < 10000)) {
...
async function processResults(results) {
  ...
  const critical = results.filter(pos => !pos.liquidatable && pos.hf < 1.02 && pos.hf > 0 && pos.debt > 1000);
...
async function sendDiscord(message, urgent = false) {
  ...
  try {
    await fetch(DISCORD_WEBHOOK, {
      ...
      body: JSON.stringify({ content: urgent ? '@here ' + message : message, username: '⚡ Liquidator V6.1' }),
    });
  } catch {}
}
...
async function main() {
  ...
  await sendDiscord(`🚀 LIQUIDATOR V6.1 STARTED\n📊 ${totalPositions} positions\n🛡️ Bad Debt Filter: ON\n🔗 Aave | Compound | Venus\n💰 Min profit: $${MIN_PROFIT_USD}`, true);

  console.log('🚀 Listening for events...\n');

  setInterval(() => {
    console.log(`[${new Date().toLocaleTimeString()}] Events: ${stats.events} | Checks: ${stats.checks} | BadDebt: ${stats.badDebt} | Liquidations: ${stats.liquidations}`);
  }, 60000);
...
EOFcat > scripts/eventLiquidatorV6.js << 'EOF'
import 'dotenv/config';
import { ethers } from 'ethers';
import fs from 'fs';
