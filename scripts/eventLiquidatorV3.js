...
const CHAIN_ASSETS = {
  base: [
    ...
    { symbol: 'USDbC', token: '0xd9aAEc86B65D86f6A7B5B1b0c42FFA531710b6CA', aToken: '0x0a1d576f3eFeF75b330424287a95A366e8281D54', debtToken: '0x7376b2F323dC56fCd4C191B34163ac8a84702DAB', decimals: 6 },
    { symbol: 'wstETH', token: '0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452', aToken: '0x99CBC45ea5bb7eF3a5BC08FB1B7E56bB2442Ef0D', debtToken: '0x41A7C3f5904ad176dACbb1D99101F59ef0811DC1', decimals: 18 },
    ...
    { symbol: 'weETH', token: '0x04C0599Ae5A44757c0af6F9eC3b93da8976c150A', aToken: '0x7C307e128efA31F540F2E2d976C995E0B65F51F6', debtToken: '0xB8C0c7A1BB7F39dde2515BDe199a9B7eb3A61Bf5', decimals: 18 },
    { symbol: 'cbBTC', token: '0xcbB7C0000aB88B473b1f5aFd9ef808440eed33Bf', aToken: '0x59CE2cBF1F65f22D8b35B2B6F007a9E3BB0B0c0D', debtToken: '0x2a0aFe2Cf8C8B938c38c11Bf96e7D1A1E8195125', decimals: 8 },
  ],
  polygon: [
    ...
    { symbol: 'USDT', token: '0xc2132D05D31c914a87C6611C10748AEb04B58e8F', aToken: '0x6ab707Aca953eDAeFBc4fD23bA73294241490620', debtToken: '0xfb00AC187a8Eb5AFAE4eACE434F493Eb62672df7', decimals: 6 },
    { symbol: 'WBTC', token: '0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6', aToken: '0x078f358208685046a11C85e8ad32895DED33A249', debtToken: '0x92b42c66840C7AD907b4BF74879FF3eF7c529473', decimals: 8 },
  ],
  arbitrum: [
    ...
    { symbol: 'USDT', token: '0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9', aToken: '0x6ab707Aca953eDAeFBc4fD23bA73294241490620', debtToken: '0xfb00AC187a8Eb5AFAE4eACE434F493Eb62672df7', decimals: 6 },
    { symbol: 'WBTC', token: '0x2f2a2543B76A4166549F7aaB2e75Bef0aefC5B0f', aToken: '0x078f358208685046a11C85e8ad32895DED33A249', debtToken: '0x92b42c66840C7AD907b4BF74879FF3eF7c529473', decimals: 8 },
    { symbol: 'ARB', token: '0x912CE59144191C1204E64559FE8253a0e49E6548', aToken: '0x6533afac2E7BCCB20dca161449A13A32D391fb00', debtToken: '0x44705f578135cC5d703b4c9c122528C73Eb87145', decimals: 18 },
  ],
  avalanche: [
    ...
    { symbol: 'WETH', token: '0x49D5c2BdFfac6CE2BFdB6640F4F80f226bc10bAB', aToken: '0xe50fA9b3c56FfB159cB0FCA61F5c9D750e8128c8', debtToken: '0x0c84331e39d6658Cd6e6b9ba04736cC4c4734351', decimals: 18 },
    { symbol: 'WBTC', token: '0x50b7545627a5162F82A992c33b87aDc75187B218', aToken: '0x078f358208685046a11C85e8ad32895DED33A249', debtToken: '0x92b42c66840C7AD907b4BF74879FF3eF7c529473', decimals: 8 },
  ],
};
...
// ============================================================
// SMART COLLATERAL/DEBT DETECTION
// ============================================================
  ...
  try {
    ...
    return results.map((r, i) => {
      ...
      try {
        ...
        const collateralUsd = Number(d[0]) / 1e8;
        const debtUsd = Number(d[1]) / 1e8;
        const hf = Number(d[5]) / 1e18;
        return { user: users[i], debt: debtUsd, collateral: collateralUsd, hf, liquidatable: hf < 1.0 && hf > 0 && debtUsd > 100, protocol: 'aave', chain };
      } catch { return null; }
    }).filter(Boolean);
  } catch { return []; }
}
  ...
  try {
    if (protocol === 'aave') {
      ...
      if (!collateral?.asset || !debtAsset?.asset) {
        console.log(`   ❌ Could not detect collateral/debt assets`);
      ...
      console.log(`   💰 Covering: ${(debtAmount * 0.5).toFixed(4)} ${debtAsset.symbol}`);
    ...
    await sendDiscord(`❌ LIQUIDATION FAILED: ${e.message.slice(0, 100)}`, false);
